...
# Instead of replacing one by one, let's just generate the entire EN block!
import json
...
# The EN part currently has BOTH the UI keys and the auto-generated desc/extra keys.
# Let's extract only the desc_ and extra_ keys from the existing EN part.

desc_extra = []
# Find all desc_ and extra_ keys using a safe regex that skips values with quotes
matches = re.finditer(r"      '(desc_key_[^']+)'\s*:\s*'(.*?)',\n", en_part)
for match in matches:
    desc_extra.append(match.group(0))

matches2 = re.finditer(r"      '(extra_key_[^']+)'\s*:\s*'(.*?)',\n", en_part)
...
for k, v in en_dict.items():
    # Because we are building the text from scratch, we can just replace ' with \'
    v_escaped = v.replace("'", "\\'")
    new_en_part += f"      '{k}': '{v_escaped}',\n"

for line in desc_extra:
    new_en_part += line

new_en_part += "}\n  };\n\n  static String str"
new_text = id_part + "'en': {" + new_en_part + parts[1].split('  static String str')[1]

with open('lib/core/app_strings.dart', 'w', encoding='utf-8') as f:
    f.write(new_text)

print("REBUILT EN BLOCK")
