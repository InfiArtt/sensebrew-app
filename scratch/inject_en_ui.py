...
# We need to insert these strings right before the end of the 'en' block
# Find the end of 'en' block
end_en = text.rfind('}', 0, text.rfind('static String str'))

# Let's verify end_en is correct. The text has:
#    }
#  };
#  static String str...

# Actually, the block closes with:
#    }
#  };

# So we can just replace '    }\n  };\n\n  static String str' with our injected keys
# wait, better to use regex to find the closing brace of 'en'
...
# Just replace `      'extra_key_15': '...',\n    }\n  };`
# But what if there's no trailing comma?
import re
new_text = re.sub(r"(      'extra_key_15': '.*?',?\n)(\s*\}\s*;\s*static String str)", r"\1" + insertion + r"\2", text)

with open('lib/core/app_strings.dart', 'w', encoding='utf-8') as f:
    f.write(new_text)

print("Injected EN UI strings!")
