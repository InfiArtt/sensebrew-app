...
# We just need to fix literal newlines that are inside single-quoted strings.
# A simple way is to find all 'calib_guide' in the en block and fix it.
# Or just find any newline that doesn't have a space or quote or brace at the start of the line, wait no.
# Let's just do a pass on the en block.

parts = text.split("'en': {")
id_part = parts[0]
en_part = parts[1]

# We know the keys that have issues: calib_guide, gemini_help_content, groq_help_content, custom_recipe_btn, ai_recipe_btn, brew_title, brew_sec.
keys_with_newlines = ['calib_guide', 'gemini_help_content', 'groq_help_content', 'custom_recipe_btn', 'ai_recipe_btn', 'brew_title', 'brew_sec']

# Let's re-inject the EN block from scratch.
# We will just replace the entire EN block with the correct one.
import json
...
# Find all keys in en_part up to '}'
# It's better to just replace the keys in en_part by splitting string.
for k, v in en_dict.items():
    # Since we are modifying the literal text, we can use simple string replacement 
    # instead of re.sub which parses escape characters in the replacement string.
    
    # regex to find the value block for a key
    pattern = re.compile(r"'" + k + r"':\s*'(.*?)'", re.DOTALL)
    
    match = pattern.search(en_part)
    if match:
        v_escaped = v.replace("'", "\\'")
        en_part = en_part[:match.start()] + f"'{k}': '{v_escaped}'" + en_part[match.end():]

new_text = id_part + "'en': {" + en_part
