...
# We need to replace the 'en' block. Let's find it.
...
# Create a list of all lines in en_text, keeping non-desc_key and non-extra_key lines intact
new_lines = []
for line in en_text.split('\n'):
    m = re.search(r"^\s*'(desc_key_\d+|extra_key_\d+)':", line)
    if not m:
        # keep standard keys (if any are split on multiple lines, this logic fails, 
        # but standard keys are on single lines). Wait! My previous script broke desc_key into multiple lines!
        # So we can't just iterate by line.
        pass

# Better approach: The EN block should mirror the ID block!
# Let's extract all non-dynamic keys from EN block by regex matching them BEFORE the corruption?
# But we can just use the ID block as a template!
id_block_match = re.search(r"('id': \{)(.*?)(\n    \},)", content, re.DOTALL)
id_text = id_block_match.group(2)

# Copy id_text, and replace all its values with EN values
new_en_text = id_text

# Standard UI strings to translate manually
ui_translations = {
...
# Combine ui_translations and trans_new
all_trans = {**ui_translations, **trans_new}

# Now we replace line by line from id_text
output_lines = []
for line in id_text.split('\n'):
    ...
    if m:
        ...
        if key in all_trans:
            ...
            output_lines.append(line) # fallback to ID if missing
    else:
